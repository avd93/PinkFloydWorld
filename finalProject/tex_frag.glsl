...
void main() {
    ...
    float spec = pow(max(dot(viewDirection,reflectDirection),0.0),specIntensity);
    vec3 specular = (0.5f)*spec*lightColor;

//   color_out = texture(tex,fuv).bgra;
//    color_out = texture(tex,fuv).rgba;
//  color_out = vec4(fuv,0,1);
//    color_out = vec4(fcolor,1);
    color_out = vec4((ambientStrength + diff +  specular), 1)*texture(tex,fuv).bgra;
}
